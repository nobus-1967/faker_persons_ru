...
| `--filetype`, `-f` | строка, выбор из вариантов: `csv`, `xlsx`, `sql`, `sqlite3`, `mariadb` | Расширение (тип) генерируемого файла/файлов (`CSV`-файл, файл `Microsoft Excel` версии 2007 года и новее, `SQL`-файл для импорта в различные реляционные СУБД, готовая база данных СУБД `SQLite3` и `MariaDB`-файл для импорта в СУБД MySQL/MariaDB). *Если параметр не задан, то часть массива сгенерированных данных отображается только на экране.* **Параметр может быть указан несколько раз для создания файлов различных типов!**                                                                                                                          |
