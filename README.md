...
| `--data`, `-d`     | строка, выбор из вариантов: `base`, `contacts`              | Генерируемая программой фейковая информация: `base` (базовая &mdash; Ф.И.О., пол, дата рождения) или `contacts` (базовая плюс контакты &mdash; номер телефона и адрес email). По умолчанию программа генерирует только базовую информацию, параметр `base` в этом случае можно не указывать.                                                                                        |
...
2. `python[3] -m faker_persons_ru --total 100 --data contacts` &mdash; генерация и частичный вывод на экран массива из `100` записей (базовая информация и контакты);

3. `python[3] -m faker_persons_ru --total 5000 --data contacts --output my_dataset --fyletype csv` &mdash; генерация и частичный вывод на экран `5000` записей (базовая информация и контакты) с сохранением массива данных в файл `my_dataset.csv` в домашней папке пользователя;

4. `python[3] -m faker_persons_ru -t 10_000 -d contacts -o 'big dataset' -f csv -f sqlite3` &mdash; генерация и частичный вывод на экран `10000` записей (базовая информация и контакты) с сохранением массива данных в файлы `big dataset.csv` и `big dataset.sqlite3` в домашней папке пользователя;
